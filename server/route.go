...
func NewRouter(r *gin.Engine, db *pgxpool.Pool) {
	...
	controllers := r.Group("/v1")
	{
		...
		department := controllers.Group("/department")
		{
			...
			department.GET("", middleware.ContentType, middleware.Authorization, deptHandler.GetAll)
		...
		employee := controllers.Group("/employee")
		{
			...
			employee.PATCH(":identityNumber", middleware.Authorization, employeeHdlr.Update)
